...
**Target**: 5-package monorepo delivering V1 for Node.js + Express + pino + AsyncLocalStorage.
...
- `EventAPI` - add, child, error, annotate, flush
...
- Provides user-facing `event` and `timers` APIs
...
- Verify: fields captured, timers work, errors normalized, sampling decisions, redaction applied, flush receipt accurate
