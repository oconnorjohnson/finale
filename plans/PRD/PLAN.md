...
| --------------------------------------------- | ---------------------------- | ------- | ---------- |
| **stdout + platform log agent** (recommended) | High (agent handles retries) | Low     | Low        |
...
| **Fan-out (request triggers N internal calls)** | Still one primary event. Internal calls can add fields and bounded sub-events.     |
...
**V1**: Sub-events are supported as **embedded milestones** within the primary event.

This is designed for long-running and LLM-oriented workflows where important checkpoints happen before final completion.

- Sub-events are attached to the in-flight primary event (not emitted separately).
- They are bounded and budget-aware to prevent runaway payload growth.
- Timers remain the default for pure duration tracking; sub-events are for semantic milestones.

**Deferred to v2+**: Out-of-band milestone emission (`emitMilestone()` as separate records) for teams that need multi-record streaming during request execution.
...
```typescript
{
  ...
  // Sub-events (optional, bounded)
  "sub_events": [
    {
      "name": "llm.plan.generated",
      "at_ms": 125,
      "fields": { "llm.model": "gpt-4.1", "llm.tokens_in": 1880 }
    },
    {
      "name": "tool.call.completed",
      "at_ms": 271,
      "fields": { "tool.name": "search", "tool.duration_ms": 41 }
    }
  ],
...
// Add semantic milestone to embedded sub-events (capped + budget-aware)
scope.event.subEvent('llm.chunk.received', {
  'llm.tokens_out': 512,
  'llm.provider': 'openai',
});
...
| `scope.event.add()` / `.child()` / `.error()` / `.annotate()` / `.subEvent()` | V1     |
...
| Out-of-band milestone emission    | Embedded sub-events cover V1 needs   |
...
## Appendix A. Product Outcome Contract (Rich Logging First)

finale should be evaluated first as an observability instrumentation product, not as an OSS packaging exercise.

### Primary V1 Outcomes

1. **Business-context quality**: request and workflow events carry enough domain context to explain business outcomes, not just technical failures.
2. **LLM observability without hosted complexity**: teams get meaningful model/tool/step visibility while staying on their own infrastructure.

### Simplicity Principle

The default path must stay small and predictable:

- one primary event per request/workflow
- low-friction `.add()` enrichment from anywhere in scope
- one clear flush point

Flexibility is allowed only where it preserves this default ergonomic model.

### Canonical Questions V1 Must Answer

Given one retained event (plus optional milestone events), operators should answer:

- What happened for this request/workflow, and was the outcome successful?
- Which business entity or tenant was affected?
- Which dependency step failed or regressed?
- Which feature flag/release cohort influenced behavior?
- For LLM workflows: which step/tool/model consumed tokens/cost and where did failure occur?

---

## Appendix B. V1 Context Taxonomy and Coverage Contract

V1 is complete only when all context families below are first-class and documented:

1. **HTTP lifecycle**: route, method, status, latency, request identity metadata.
2. **Identity and tenant context**: user/org/tenant/access shape (with safety policy).
3. **Dependency operations**: database, queue, and external API outcomes and timings.
4. **Feature and release context**: flag decisions, experiment cohorts, deployment version.
5. **LLM workflow context**: step milestones, model/provider, token usage, cost estimates, tool calls.
6. **Error and retry context**: structured error taxonomy, retry counts, terminal failure reason.
7. **Runtime and deploy context**: env, service version, region/runtime metadata.

Each context family should include:
...
## Appendix C. LLM Event Granularity Contract (Simple by Default)

V1 should support two LLM observability modes with explicit guidance.

### Default Mode

**Single primary event with embedded milestones** remains the default for request-aligned workflows.

Why this exists: most teams need richer workflow insight without moving to a multi-record orchestration model.

### Optional Mode

**Single primary event plus optional out-of-band milestone emission** is available for long-running, interactive, or streaming workflows where mid-flight visibility is operationally required.

### Guardrails

- Optional milestone emission is opt-in and off by default.
- Primary-event semantics remain authoritative for request/workflow outcome.
- Optional mode must not require hosted control plane assumptions.

---

## Appendix D. Compatibility, Release, and Support Commitments

To support OSS adoption, finale should publish explicit runtime and release policy.

### Compatibility and Support Matrix

- Node.js is Tier 1 runtime for V1.
- Edge runtimes are supported via explicit scoping patterns with best-effort guarantees.
- TypeScript minimum supported version and tested range are documented and tested in CI.
- Integration compatibility is explicitly stated for Express/Next.js (Node runtime) and pino sink path.
